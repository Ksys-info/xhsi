...
//                udp_receiver.start();
    ...
    private void init() throws Exception {
        ...
        CmdConfigurator cmd_conf = new CmdConfigurator(this);
        ...
        for (int win_index = 0 ; win_index < PreferencesDialog.win_count() ; win_index++) {
            ...
            if (win_index >= XHSIInstrument.CMD_ID) {
                du.set_name(cmd_conf.getName(win_index - XHSIInstrument.CMD_ID));
            }
        ...
        cmd_conf.setModelFactory(model_instance);

        // create user interface
        create_UI();

        // Tell Command configurator that the system in up
        cmd_conf.ready();
        ...
        for (int i=0; i<instruments.size(); i++) {
            switch (instruments.get(i).get_index()) {
                ...
                default : // XHSIInstrument.CMD_ID ... XHSIInstrument.CMD_ID+8
                    ...
                    this.preferences.add_subsciption(cmd_ui, XHSIPreferences.PREF_BORDER_STYLE);
                    this.preferences.add_subsciption(cmd_ui, XHSIPreferences.PREF_BORDER_COLOR);
                    this.preferences.add_subsciption(cmd_ui, XHSIPreferences.PREF_DU_PREPEND);
                    break;
            }
        }
...
//taxi.get_chart("YMML");
    ...
    public ArrayList<XHSIInstrument> getInstruments() {
    ...
    public void setAlwaysOnTop(boolean value) {
        for (int i=0; i<instruments.size(); i++) {
            XHSIInstrument inst = instruments.get(i);
            if (inst.du.enabled()) {
                inst.frame.setAlwaysOnTop(value);
            }
        }
