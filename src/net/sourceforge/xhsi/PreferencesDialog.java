...
public class PreferencesDialog extends JDialog implements ActionListener {
    ...
    static int win_count() {
        return FIXED_WINS + CmdConfigurator.getWindowCount();
    }

    PropertiesPanel cmd_panel;
    ...
    private JCheckBox dim_range_arcs_checkbox;
    private JCheckBox small_aircraft_symbol_checkbox;
...
//        }
        ...
        this.dim_range_arcs_checkbox.setSelected(preferences.get_preference(XHSIPreferences.PREF_DIM_RANGE_ARCS).equalsIgnoreCase("true"));

        this.small_aircraft_symbol_checkbox.setSelected(preferences.get_preference(XHSIPreferences.PREF_SMALL_AIRCRAFT_SYMBOL).equalsIgnoreCase("true"));
...
//        dialog_line++;
    ...
    private JPanel create_windows_tab() {
        ...
        for (int win_index = 0 ; win_index < PreferencesDialog.win_count() ; win_index++) {
            XHSIInstrument.DU instrum = XHSIInstrument.DU.values()[win_index];
...
//        dialog_line++;
    ...
    private JPanel create_nd_options_tab() {
        ...
        cons.gridy = dialog_line;
        cons.anchor = GridBagConstraints.EAST;
        nd_options_panel.add(new JLabel("Dim range arcs", JLabel.TRAILING), cons);
        cons.gridx = 2;
        cons.gridwidth = 1;
        cons.gridy = dialog_line;
        cons.anchor = GridBagConstraints.WEST;
        this.dim_range_arcs_checkbox = new JCheckBox();
        nd_options_panel.add(this.dim_range_arcs_checkbox, cons);
        dialog_line++;

        // Small heading pointer
        cons.gridx = 0;
        cons.gridwidth = 1;
        cons.gridy = dialog_line;
        cons.anchor = GridBagConstraints.EAST;
        nd_options_panel.add(new JLabel("Small aircraft symbol", JLabel.TRAILING), cons);
        cons.gridx = 2;
        cons.gridwidth = 1;
        cons.gridy = dialog_line;
        cons.anchor = GridBagConstraints.WEST;
        this.small_aircraft_symbol_checkbox = new JCheckBox();
        nd_options_panel.add(this.small_aircraft_symbol_checkbox, cons);
        dialog_line++;
...
//        dialog_line++;
    ...
    private JScrollPane create_cmd_options_tab() {
        ...
        cmd_panel = new PropertiesPanel(
            conf.getEntryTable(),
            conf.getProperties(),
            new PropertiesPanel.Done() {
                public void done(java.util.Properties props) {
                    conf.setProperties(props, cmd_panel);
                }
            }
        );
...
//                this.preferences.set_preference(XHSIPreferences.PREF_SIMCOM, borderstyles[this.simcom_combobox.getSelectedIndex()]);
            ...
            if ( this.dim_range_arcs_checkbox.isSelected() != this.preferences.get_preference(XHSIPreferences.PREF_DIM_RANGE_ARCS).equals("true") )
                this.preferences.set_preference(XHSIPreferences.PREF_DIM_RANGE_ARCS, this.dim_range_arcs_checkbox.isSelected()?"true":"false");

            if ( this.small_aircraft_symbol_checkbox.isSelected() != this.preferences.get_preference(XHSIPreferences.PREF_SMALL_AIRCRAFT_SYMBOL).equals("true") )
                this.preferences.set_preference(XHSIPreferences.PREF_SMALL_AIRCRAFT_SYMBOL, this.small_aircraft_symbol_checkbox.isSelected()?"true":"false");
