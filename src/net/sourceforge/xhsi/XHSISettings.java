...
//            this.fuel_dialog.pack();
    ...
    public void mapZoomIn() {
        if (map_range_index == 0) {
            if (map_zoomin) {
                return;
            } else {
                map_zoomin = true;
                map_range_index = zoomin_range_list.length;
            }
        }
        this.map_range_index--;
        this.avionics.set_range_index(map_range_index);
        this.avionics.set_zoomin(map_zoomin);
    }

    public void mapZoomOut() {
        if (map_range_index == range_list.length - 1) {
            if (!map_zoomin) {
                return;
            } else {
                map_zoomin = false;
    ...
    public String getZoomRange(int index, boolean zoomin) {
      //return map_zoomin ? zoomin_range_list[map_range_index] : range_list[map_range_index];
        return zoomin ? zoomin_range_list[index] : range_list[index];
    }

    public void setSymbols(boolean a, boolean b, boolean c, boolean d, boolean e, boolean f, boolean g) {
        checkbox_symbols_show_arpt.setSelected(a);
        checkbox_symbols_show_wpt.setSelected(b);
        checkbox_symbols_show_vor.setSelected(c);
        checkbox_symbols_show_ndb.setSelected(d);
        checkbox_symbols_show_tfc.setSelected(e);
        checkbox_symbols_show_pos.setSelected(f);
        checkbox_symbols_show_data.setSelected(g);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_ARPT);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_WPT);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_VOR);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_NDB);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_TFC);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_POS);
        actionPerformed(XHSISettings.ACTION_SYMBOLS_SHOW_DATA);
    }


    public void toggleSymbol(String symbol) {
        JCheckBoxMenuItem cb = null;
        if      (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_ARPT)) cb = checkbox_symbols_show_arpt;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_WPT )) cb = checkbox_symbols_show_wpt;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_VOR )) cb = checkbox_symbols_show_vor;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_NDB )) cb = checkbox_symbols_show_ndb;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_TFC )) cb = checkbox_symbols_show_tfc;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_POS )) cb = checkbox_symbols_show_pos;
        else if (symbol.equals(XHSISettings.ACTION_SYMBOLS_SHOW_DATA)) cb = checkbox_symbols_show_data;
        if (cb != null) {
            cb.setSelected(!cb.isSelected());
            actionPerformed(symbol);
        }
    }
